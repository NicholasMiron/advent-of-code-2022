...
class Monkey {
  constructor(info) {
    ...
    this.operation = info.operation.bind(this);
    this.test = () => !(this.worryLevel % info.testVal);
    this.pass = info.pass;
    this.fail = info.fail;
  ...
  inspect() {
    ...
    while(this.items.length) {
      ...
      this.worryLevel = ~~(this.operation(itemWorryLevel) / 3);
...
const solution = (input) => {
  ...
  for (let i = 0; i < 20; i++) {
    monkeys.forEach((monkey) => {
      const distribution = monkey.inspect();
