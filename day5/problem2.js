...
const solution = (input) => {
  ...
  commands.forEach((command) => {
    stacks[command.to].push(...stacks[command.from].slice(stacks[command.from].length - command.qty));
    stacks[command.from] = stacks[command.from].slice(0, stacks[command.from].length - command.qty);
  });
